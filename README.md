...
**Tour of Japan の全ステージ**に加え、**全日本自転車競技選手権大会 個人TT・チームTT**（2026 宮崎ほか）にも対応します。レースのデータ構造（タイム精度・フェーズ表記）から種別を自動判定し、個人タイムトライアル・マススタート（ロード/クリテリウム）・チームTTのそれぞれに最適な表示へ切り替えます。マススタートでは着順（公式の「位」）・トップ差・周回遅れ・スプリント通過に加え、**逃げ／メイン集団／遅れの集団状況やトップとの差をライブ表示**します。
...
- **全日本選手権 個人TT・チームTT**（コース 1周 = 14.2km と推定）：個人TT＝男子エリート 42.6km(3周)／男子U23 28.4km(2周)／女子E+U23 14.2km(1周)、チームTT＝28.4km(**2周**)。チームTTは ctg が英字コードのため、ページ・タイトルの「チームTT」表記で自動判定します。自動判定が外れても上部の **個人 / チーム** トグルで切替できます（当日の実カテゴリ表記に合わせて確認）。
