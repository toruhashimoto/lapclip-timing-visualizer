...
**Tour of Japan の全ステージ**に加え、**全日本自転車競技選手権大会（ロードレース・個人TT・チームTT）**にも対応します。レースのデータ構造（タイム精度・フェーズ表記）から種別を自動判定し、個人タイムトライアル・マススタート（ロード/クリテリウム）・チームTTのそれぞれに最適な表示へ切り替えます。マススタートでは着順（公式の「位」）・トップ差・周回遅れ・スプリント通過に加え、**逃げ／メイン集団／遅れの集団状況やトップとの差をライブ表示**します。
...
| **iPhone / iPad** | **Safari ブックマークレット** | ✅ β（拡張不要・1タップ） |
...
2. ユーザースクリプトをインストール：
   - **方法A（配布・推奨）**：[最新リリース](https://github.com/toruhashimoto/lapclip-timing-visualizer/releases/latest)の `lapclip-visualizer.user.js` を開く。
     - Tampermonkey がインストール確認を出せばそのまま **Install**。
     - ブラウザがファイルを**ダウンロード**してしまう場合（GitHub が添付配信するため）は、ダウンロードした `.user.js` を **Tampermonkey ダッシュボードにドラッグ＆ドロップ**（または `file://` で開く）。
     - **v1.0.0 以降は一度入れれば自動更新**（`@updateURL` で最新リリースを追従）。最初の1回だけ手動です。
...
## インストール（iPhone / iPad / Safari ブックマークレット・β・推奨）

拡張不要。PC のブックマークレットと**同じコード**を Safari の「お気に入り」に保存し、1タップで起動します。iOS Safari はアドレスバーに `javascript:` を直接入力できないため、**ブックマークを編集してコードを貼り付ける**形で導入します（端末で1回だけ）。

1. [`ios-bookmarklet/bookmarklet.txt`](ios-bookmarklet/bookmarklet.txt) の `javascript:…`（1行）を全文コピー。
2. Safari で任意のページを「ブックマークを追加」で保存。
3. ブックマーク編集 → 名前を「LapClip Visualizer」に、**URL 欄をコピーした `javascript:…` で置き換え**て保存。
4. LapClip の `result.php` を開き、保存したブックマーク（お気に入り）をタップ → オーバーレイ表示。

> 詳しい手順・ショートカット版との違い・制限は [`ios-bookmarklet/README.md`](ios-bookmarklet/README.md)。
...
URL ではなく**ページの内容**（タイム精度・フェーズ表記・ギャップ書式・ページタイトル）から種別を判定するため、TOJ の全ステージや**全日本選手権 個人TT・チームTT**、さらに将来の大会・年度でも動作します。上部の **個人 / チーム** トグルで手動上書きも可能です。

| 種別 | 例 | 判定の手がかり | 表示 |
|---|---|---|---|
| **個人TT** | 堺ステージ | 1/100秒精度・`中間点`/`FINISH` | F1予選風タイミングタワー（中間点＋Finish＋トップ差） |
| **マススタート** | 京都・いなべ等のロード、堺クリテリウム、**全日本ロードレース**（新潟・三国川ダム周回, 第94回） | `N周`/`X/Y周`・秒精度・集団同タイム | 着順（公式の「位」）＋トップ差＋周回遅れ（`-N周`）＋スプリント通過（`SPn`）。加えて **逃げ／メイン集団／遅れ** の集団状況をライブ表示 |
| **チームTT** | 大鹿ステージ（`ctg=004`）／**全日本チームTT** | カテゴリ（`ctg=004`）またはページ・タイトルの「チームTT / Team Time Trial / TTT」表記 | チーム別に 順位 / Lap / Finish / トップ差 |
...
- **全日本選手権ロードレース**（第94回＝**新潟県南魚沼市・三国川ダム周回コース／1周 12km**。男子エリート＝12km×15周＝180km、他カテゴリは周回数が異なる）はマススタートとして判定・表示します。総周回数はページの `X/Y周` 表記から読み取り、固定値はハードコードしません（会場・距離・周回数は年度毎に異なるため当日確認）。
- **全日本選手権 個人TT・チームTT**：会場・距離・周回数は**年度毎に変わるためレース当日に実ページで確認**します（例：2026 は宮崎／1周 14.2km）。チームTTは ctg が英字コードのため、ページ・タイトルの「チームTT / Team Time Trial / TTT」表記で自動判定し、外れても上部の **個人 / チーム** トグルで切替できます。
...
SPA プレビューでは `?mock=criterium` / `?mock=road` / `?mock=live` / `?mock=jprr` で各マススタート表示を確認できます（合成データ）。`?mock=jprr` は全日本ロード想定で、カテゴリタブ（Men Elite / MM / Men U23 / Women Elite+WU23 / WM）を切り替えながら、周回遅れ（`-N周`）と集団状況（逃げ／メイン集団／遅れ）を含む表示を事前確認できます（周回数の判定は実ページ DOM 依存のまま。モックは表示確認専用）。
